...
document.addEventListener("click", event => {
    ...
    if (element.className.includes("hide")) {
        ...
        if (element.parentElement.className == "r+odvoz") {
            ...
            if (h_odpad_visible.includes(id)) {
                // delete the id from r_odpad_visible, remove the order and subtract one from c_visible
                const index = h_odpad_visible.indexOf(id)
                if (index > -1){
    ...
    setInterval(function(){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if (r_odvoz.length != 0 || h_odvoz.length != 0 || r_dovoz.length != 0 || h_dovoz.length != 0 || r_odpad != 0 || h_odpad != 0) {
                    ...
                    // run 6 for loops that add information for each order from both tables
